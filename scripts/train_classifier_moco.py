...
def train():
    ...
    moco_epoch = 20
    ckpt_path = f"checkpoints/checkpoints_moco/moco_resnet18_epoch{moco_epoch}.pth"

    print(f"\nLoading MoCo checkpoint: {ckpt_path}\n")
    state_dict = torch.load(ckpt_path, map_location=device)
    encoder.load_state_dict(state_dict)

    # Remove MLP head for downstream classification
    ...
    for epoch in range(5):
        ...
        val_loss, val_acc, val_f1 = evaluate(encoder, head, val_loader, criterion, device)
        print(f"Linear Epoch {epoch+1}/5  val_loss={val_loss:.4f}  val_acc={val_acc:.4f}  val_f1={val_f1:.4f}")
    ...
    optimizer = torch.optim.Adam(list(encoder.parameters()) + list(head.parameters()), lr=5e-5)
    ...
    for epoch in range(20):
        ...
        val_loss, val_acc, val_f1 = evaluate(encoder, head, val_loader, criterion, device)
        print(f"FT Epoch {epoch+1}/20  val_loss={val_loss:.4f}  val_acc={val_acc:.4f}  val_f1={val_f1:.4f}")
    ...
    if best_state is not None:
        torch.save(best_state, "checkpoints/checkpoints_moco/best_ft_model.pth")
        print("Saved → checkpoints/checkpoints_moco/best_ft_model.pth")
